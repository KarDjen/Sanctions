...
# Function to apply regex patterns and extract details
def extract_details(text, patterns):
    details = []
    for pattern in patterns:
        match = pattern.search(text)
        if match:
            details.append(match.groupdict())
    return details

# Function to fetch and parse legal representation
def fetch_and_print_tags(uri, not_found_uris, match_count, patterns):
    ...
    if header_tag:
        ...
        if p_tags_with_keywords:
            ...
            for tag in p_tags_with_keywords:
                ...
                details = extract_details(tag, patterns)
                if details:
                    for detail in details:
                        print("Extracted Details:", detail)
            print("------------------------------------------------------")
...
with ThreadPoolExecutor(max_workers=10) as executor:
    futures = {executor.submit(fetch_and_print_tags, uri, not_found_uris, match_count, patterns): uri for uri in
               test_uris}
